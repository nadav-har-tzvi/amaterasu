...
Feature: Support of creating a new Amaterasu job repository
  ...
  Scenario: Invoking the InitRepository handler with a valid path should result in a new git repository
    ...
    Then A directory with path "tmp/amaterasu" should be created
    And The directory in path "tmp/amaterasu" should be a git repository
    And The "tmp/amaterasu" directory should have a "maki.yml" file
    And The "tmp/amaterasu" directory should have a "src" subdirectory
    And The "tmp/amaterasu" directory should have a "env" subdirectory
    And the "tmp/amaterasu/env" directory should have a "default" subdirectory
    And the "tmp/amaterasu/env/default" directory should have a "job.yml" file
    And the "tmp/amaterasu/env/default" directory should have a "spark.yml" file
  ...
  Scenario: Invoking the InitRepository handler with a valid path that is already a empty repository should create all the required Amaterasu file structure
    ...
    When InitRepository handler is invoked with the given path
    Then The directory in path "tmp/amaterasu" should be a git repository
    And The "tmp/amaterasu" directory should have a "maki.yml" file
    And The "tmp/amaterasu" directory should have a "src" subdirectory
    And The "tmp/amaterasu" directory should have a "env" subdirectory
    And the "tmp/amaterasu/env" directory should have a "default" subdirectory
  ...
  Scenario: Invoking the InitRepository handler with a valid path that is already a repository that is missing a maki file, should only create the maki file
    ...
    Given The "tmp/amaterasu/env/default" directory has a "job.yml" file
    Given The "tmp/amaterasu/env/default" directory has a "spark.yml" file
    When InitRepository handler is invoked with the given path
    Then The "tmp/amaterasu" directory should have a "maki.yml" file
  ...
  Scenario: Invoking the InitRepository handler with a valid path that is already a repository that is missing the env directory, should only create the env directory
    ...
    Given The path is a repository
    Given The "tmp/amaterasu" directory has a "src" subdirectory
    Given The "tmp/amaterasu" directory has a "maki.yml" file
    When InitRepository handler is invoked with the given path
    Then The "tmp/amaterasu" directory should have a "env" subdirectory
    And the "tmp/amaterasu/env" directory should have a "default" subdirectory
    And the "tmp/amaterasu/env/default" directory should have a "job.yml" file
    And the "tmp/amaterasu/env/default" directory should have a "spark.yml" file
